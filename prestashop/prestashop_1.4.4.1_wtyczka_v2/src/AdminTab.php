...
abstract class AdminTabCore {
	...
	public function postProcess() {
		...
		/* Manage list filtering */ elseif (Tools::isSubmit('submitFilter' . $this->table) OR $cookie->{'submitFilter' . $this->table} !== false) {
			...
			foreach ($this->_list AS $tr) {
				...
				if ($this->edit OR $this->delete OR ($this->view AND $this->view !== 'noActionColumn')) {
					...
					if ($_GET['tab'] == 'AdminOrders') {
						...
						if (needs_attention($id)) {
							if (isset($_POST['poprawiona']) && $_POST['poprawiona'] == 1) {
								add_info_about_invoice_fault_resolved($id);
							} else {
								echo '<div style="faktura_blad" title="Faktura moze wymagać poprawki"><img src="../img/admin/warning.gif"/><form action="" method="post"><input type="hidden" name="poprawiona" value="1"/><input type="submit" value="OK" title="Zweryfikowałem fakturę"/></form></div>';
							}
						}
						if (faktura_stworzona($id)) {
							echo '<a href="download.php?cart_order_id=' . $id . '" title="Wydrukuj fakturę"><img src="../img/admin/pdf.gif"/></a>';
							echo '<br />';
						} else {
							echo '<a href="api_request.php?cart_order_id=' . $id . '&type=invoice">Wystaw Fakturę</a>';
							echo '<br />';
							echo '<a href="api_request.php?cart_order_id=' . $id . '&type=invoice&Wysylkowa=1">Wystaw Fakturę Wysyłkową</a>';
							echo '<br />';
						}
					}
					//IFIRMA koniec
