...
if (version_compare(_PS_VERSION_, '1.5', '<')) 
	require_once dirname(__FILE__) . '/../backward_compatibility/DbQuery.php';
...
class InvoiceMap extends DataContainer{
	...
	public function save(){
		...
		if (version_compare(_PS_VERSION_, '1.5', '<')) 
		{
			$test = \Db::getInstance()->autoExecute(_DB_PREFIX_.self::TABLE_NAME, array(
	...
	public function updateInvoiceNumber($newInvoiceNumber){
		...
		if (version_compare(_PS_VERSION_, '1.5', '<')) 
		{
			\Db::getInstance()->autoExecute(_DB_PREFIX_.self::TABLE_NAME, array(
				self::COLUMN_NAME_INVOICE_NUMBER => pSQL($this->{self::COLUMN_NAME_INVOICE_NUMBER})
			), 'UPDATE', sprintf("%s = %d", self::COLUMN_NAME_ID, (int)$this->{self::COLUMN_NAME_ID}), 1 /* only one row */);
		}
		else
		{
