...
abstract class StrategyAbstract {
	...
	public function setOrder($order){
		...
		if (_PS_VERSION_ < 1.5)
			$this->_shipping = $this->_order->total_shipping;
		else
	...
	protected function _createInvoicePosition(array $product){
		...
		if (_PS_VERSION_ < 1.5)
			$invoicePosition->{InvoicePosition::KEY_CENA_JEDNOSTKOWA} = $this->_roundPrice($product['product_price_wt']);
		else
			$invoicePosition->{InvoicePosition::KEY_CENA_JEDNOSTKOWA} = $this->_roundPrice($product['unit_price_tax_incl']);

		
		$invoicePosition->{InvoicePosition::KEY_NAZWA_PELNA} = $product['product_name'];
	...
	protected function _isOrderWithoutShipping(){
		...
		if (_PS_VERSION_ < 1.5)
		{
			return ($this->_order->total_shipping == 0) ? true : false;
		}
		else
		{
