...
class AdminOrders extends AdminTab
{
	...
	public function viewDetails()
	{
		...
		if (isset($order) && is_object($order)) {
			echo '<div class="ifirma_panel" style="font-size: 18px; color: #ff0000; width: 400px;">';
   			require_once("ifirma/ifirma_functions.php");
			if(needs_attention($order->id)){
				if(isset($_POST['poprawiona']) && $_POST['poprawiona'] == 1){
					add_info_about_invoice_fault_resolved($order->id);
				}else{
			...
			if(faktura_stworzona($order->id)){
				echo '<a href="download.php?cart_order_id='. $order->id .'">Wydrukuj fakturę</a>';
				echo '<br />';
			}else{
			echo '<form action="api_request.php?cart_order_id='. $order->id .'" method="post"><input type="hidden" name="type" value = "invoice"/><input type="hidden" name="cart_order_id" value = "'. $order->id .'"/><input type="submit" value="wystaw fakture" /></form>';
			echo '<br />';
			echo '<form action="api_request.php?cart_order_id='. $order->id .'" method="post"><input type="hidden" name="type" value = "invoice"/><input type="hidden" name="cart_order_id" value = "'. $order->id .'"/><input type="hidden" name="Wysylkowa" value = "1"/><input type="submit" value="wystaw fakture wysyłkową" /></form>';
			echo '<br />';
			}
