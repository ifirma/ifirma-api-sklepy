...
class ModelModuleInvoiceMap extends Model{
	...
	const INVOICE_TYPE_BILL = 'invoice_bill';
        ...
        const API_VAT = 'ifi_invoice_api_vat';
	const API_KEY_BILL = 'ifi_invoice_api_key_bill';
	const API_KEY_INVOICE = 'ifi_invoice_api_key_invoice';
	const API_KEY_SUBSCRIBER = 'ifi_invoice_api_key_subscriber';
	const API_LOGIN = 'ifi_invoice_api_login';
	const API_HASH = 'ifi_invoice_hash';
	...
	public function updateInvoiceNumber($invoice, $newInvoiceNumber){
            ...
            if ($this->config->get($code . '_status')) {
                    ...
                    if ($quote) {
                        if(version_compare(VERSION, '1.9.0.0', '>'))
													return $this->tax->getTaxValueOC2($quote['quote'][$quote['code']]['tax_class_id'], $type);
												else
        ...
        public function getProductTax($productId, $type){
            ...
            $this->tax = new Tax( $this->registry );
						if(version_compare(VERSION, '1.9.0.0', '>'))
							return $this->tax->getTaxValueOC2($product['tax_class_id'], $type);
						else
