...
require_once dirname(__FILE__) . '/../ConnectorConfig.php';
require_once dirname(__FILE__) . '/../Utils.php';
require_once dirname(__FILE__) . '/../ConnectorResponse.php';
...
abstract class ConnectorAbstract {
	...
	protected function _getRequestHeaders($messageHash, $connectionFileType = self::DEFAULT_CONNECTION_FILE_TYPE){
		return array(
			...
			'Authentication: IAPIS user='.$this->_getConfig()->{ConnectorConfig::API_LOGIN}.', hmac-sha1='.$messageHash
		);
	}
	...
	protected function _setChangeAccountancyMonth(AccountancyMonthParam $param){
		...
		$messageHash = Utils::hmac($key, sprintf(
			...
			$this->_getConfig()->{ConnectorConfig::API_LOGIN},
			ConnectorConfig::API_KEY_SUBSCRIBBER_NAME,
			$jsonParam
		));
	...
	protected function _getCurrentAccountancyMonth(){
		...
		$rsp = curl_exec($this->_currentCurl);
		return ConnectorResponse::constructAccountancyMonthFromResponse($rsp);
	}
	...
	private function _checkResponseBeforePopulatingInvoice($data){
		...
		if(isset($data[ConnectorResponse::KEY_RESPONSE_CONTAINER]) 
			&& isset($data[ConnectorResponse::KEY_RESPONSE_CONTAINER][ConnectorResponse::KEY_CODE])
			&& !ConnectorResponse::isResponseCodeOk($data[ConnectorResponse::KEY_RESPONSE_CONTAINER][ConnectorResponse::KEY_CODE])
		){
			throw new IfirmaException(
					isset($data[ConnectorResponse::KEY_RESPONSE_CONTAINER][ConnectorResponse::KEY_INFORMATION])
					?
					$data[ConnectorResponse::KEY_RESPONSE_CONTAINER][ConnectorResponse::KEY_INFORMATION]
					:
					"Invalid response."
			);
		}
	...
	protected function _populateInvoice($rawData){
		...
		foreach ($this->_invoice->filterKeys(array_keys($data[ConnectorResponse::KEY_RESPONSE_CONTAINER])) as $key){
			switch($key){
				...
				case InvoiceAbstract::KEY_POZYCJE:
					...
					break;
				default:
					$this->_invoice->$key = $data[ConnectorResponse::KEY_RESPONSE_CONTAINER][$key];
					break;
			}
		}
	...
	private function _populateFieldContractor($data){
		...
		foreach($contractor->filterKeys(array_keys($data[ConnectorResponse::KEY_RESPONSE_CONTAINER][InvoiceAbstract::KEY_KONTRAHENT])) as $key){
			$contractor->$key = $data[ConnectorResponse::KEY_RESPONSE_CONTAINER][InvoiceAbstract::KEY_KONTRAHENT][$key];
		}
	...
	private function _populateFieldEntries($data){
		...
		foreach($data[ConnectorResponse::KEY_RESPONSE_CONTAINER][InvoiceAbstract::KEY_POZYCJE] as $positionData){
