...
class ApiManager {
	...
	private function _sendInvoice($order, $type){
		...
		$this->_comparePricesOnInvoices($newInvoiceObj, $order); // $this->_comparePricesOnInvoices($newInvoiceObj, $order["order_id"]);
	...
	private function _comparePricesOnInvoices($ifirmaInvoice, $order){
		...
		if(intval($order['total'] * 100) !== $invoiceTotal){ // if(intval($order->total_paid * 100) !== $invoiceTotal){
