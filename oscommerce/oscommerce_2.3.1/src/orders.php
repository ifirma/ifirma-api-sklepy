    ...
    echo $box->infoBox($heading, $contents);
     ...
     if (isset($oInfo) && is_object($oInfo)) {
		...
		if(faktura_stworzona($oInfo->orders_id)){
			echo '<a href="download.php?cart_order_id='. $oInfo->orders_id .'">wydrukuj fakture</a>';
		}else{
		echo '<form action="api_request.php?cart_order_id='. $oInfo->orders_id .'" method="post"><input type="hidden" name="type" value = "invoice"/><input type="hidden" name="cart_order_id" value = "'. $oInfo->orders_id .'"/><input type="submit" value="wystaw fakture" /></form>';
		echo '<form action="api_request.php?cart_order_id='. $oInfo->orders_id .'" method="post"><input type="hidden" name="type" value = "invoice"/><input type="hidden" name="cart_order_id" value = "'. $oInfo->orders_id .'"/><input type="hidden" name="Wysylkowa" value = "1"/><input type="submit" value="wystaw fakture wysyłkową" /></form>';
		}
		echo '<a href="api_configuration.php">konfiguracja api</a>';
		echo '</div>';
     }
     //IFIRMA - koniec
    echo '            </td>' . "\n";
