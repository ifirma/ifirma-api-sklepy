...
<?php
  ...
  switch ($action) {
    ...
    default:
	  ...
	  //IFIRMA - te linijki kodu naleza do wtyczki ifirma, jezeli z jakiegos powodu nie mozesz podmienic calego pliku, skopiuj we wlasciwe miejsce te linie kodu;
				if (isset($oInfo) && is_object($oInfo)) {
					require_once("ifirma/ifirma_functions.php");
					if (needs_attention($oInfo->orders_id)) {
						if (isset($_POST['poprawiona']) && $_POST['poprawiona'] == 1) {
							add_info_about_invoice_fault_resolved($oInfo->orders_id);
						} else {
							$contents[] = array('text' => '<div style="faktura_blad">Faktura moze wymagać poprawki<form action="" method="post"><input type="hidden" name="poprawiona" value="1"/><input type="submit" value="Wiem, zweryfikowałem, nie pokazuj ostrzeżenia" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary"/></form></div>');
						}
					}
					if (faktura_stworzona($oInfo->orders_id)) {
						$contents[] = array('text' => '<a class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary" href="download.php?cart_order_id=' . $oInfo->orders_id . '"><span class="ui-button-icon-primary ui-icon ui-icon-document"></span><span class="ui-button-text">Wydrukuj Fakture</span></a>');
					} else {
						$contents[] = array('text' => '<form action="api_request.php?cart_order_id=' . $oInfo->orders_id . '" method="post"><input type="hidden" name="type" value = "invoice"/><input type="hidden" name="cart_order_id" value = "' . $oInfo->orders_id . '"/><input type="submit" value="Wystaw Fakture" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary"/></form>');
						$contents[] = array('text' => '<form action="api_request.php?cart_order_id=' . $oInfo->orders_id . '" method="post"><input type="hidden" name="type" value = "invoice"/><input type="hidden" name="cart_order_id" value = "' . $oInfo->orders_id . '"/><input type="hidden" name="Wysylkowa" value = "1"/><input type="submit" value="Wystaw Fakture Wysyłkową" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary" /></form>');
					}
					$contents[] = array('text' => '<a class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary" href="api_configuration.php"><span class="ui-button-icon-primary ui-icon ui-icon-document"></span><span class="ui-button-text">Konfiguracja Api</span></a>');
				}
				//IFIRMA - koniec
