...
function get_order_summary($cart_order_id){
	$sql = tep_db_query("SELECT * FROM orders_total WHERE class = 'ot_total' AND orders_id=".mySQLSafe($cart_order_id));
	$order_summary = tep_db_fetch_array($sql);
	return $order_summary;
	
	
}
function get_order_data($cart_order_id){
	$sql = tep_db_query("SELECT * FROM orders WHERE orders_id=".mySQLSafe($cart_order_id));
	$order_summary = tep_db_fetch_array($sql);
	return $order_summary;
	
}
function get_order_ship($cart_order_id){
	$sql = tep_db_query("SELECT * FROM orders_total WHERE class = 'ot_shipping' AND orders_id=".mySQLSafe($cart_order_id));
	$order_ship = tep_db_fetch_array($sql);
	return $order_ship;
}
...
function has_ship($order_summary){
	if( ! empty( $order_summary['value'] )  && $order_summary['value']!= '0.00'){
		return true;
	}
	return false;
}
...
function add_invoice_ship_position($invoice,$order_summary){
	if( ! empty( $order_summary['value'] )  && $order_summary['value']!= '0.00'){
		...
		$total_ship = $order_summary['value'];
		$invoice_position = new PozycjaFaktury(0.23,1,$total_ship,$order_summary['title'],'usl.','PRC');
		$invoice->dodajPozycjeFaktury($invoice_position);
	}
}
...
function faktura_stworzona($cart_order_id){
	$db_query = tep_db_query("select count(*) as ile from ifirma_invoice_map where document_type = 'invoice' and cart_order_id = '$cart_order_id'");
...
function add_info_about_generated_document($document_type,$cart_order_id,$invoice_number,$invoice_type){
	global $glob;
	tep_db_query("insert into ifirma_invoice_map(document_type,cart_order_id,invoice_number,invoice_type)  values('$document_type','$cart_order_id','$invoice_number','$invoice_type')" );
	
}
function add_info_about_invoice_fault($cart_order_id){
	tep_db_query("update ifirma_invoice_map set correction_needed = 1 where cart_order_id = '$cart_order_id'");
}
function add_info_about_invoice_fault_resolved($cart_order_id){
	tep_db_query("update ifirma_invoice_map set correction_done = 1 where cart_order_id = '$cart_order_id'");
}
function needs_attention($cart_order_id){
	$db_query = tep_db_query("select count(*) as ile from ifirma_invoice_map where correction_needed = 1 and correction_done = 0 and cart_order_id = '$cart_order_id'");
	$niepoprawiona = tep_db_fetch_array($db_query); 
	return $niepoprawiona['ile'];
}
...
function pobierz_miesiac_i_rok_ksiegowy($klucz,$url,$nazwaUsera,$nazwaKlucza,$curlHandle)
{
	...
	throw new Exception("WystÄ…piÅ‚ problem z poÅ‚Ä…czeniem z ifrmÄ…. Przyczyna: ".$tab['response']['Informacja']);
}
